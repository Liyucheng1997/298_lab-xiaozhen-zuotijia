...
**🔗 在线使用:<https://zuotijia.liyucheng.me>**(默认调用本机 `http://localhost:8787` 的 Claude Code 反代，走订阅额度)
...
3. **浏览器直连本机反代 / Anthropic API**:以上都没有时，默认调用 `http://localhost:8787`（Key 填 `unused`）；也可在设置里改为官方 API。图片/PDF 在浏览器里转 base64 直接发给模型，历史记录存本浏览器。
